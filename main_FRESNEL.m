...
for ii = 1:size(cod,2)
    ...
    Q2 = exp(-1i*pi*z*lambda*(((R-M/2-1)/(M*delta)).^2+((C-M/2-1)/(M*delta)).^2));
    ...
    for L = 1:l  
        ...
        for hh = 1:n_test
            if ii<=3
                ...
                Masks   = zeros(N,N,L);
                for ll = 1:L, Masks(:,:,ll) = randsrc(N,N,cod{ii}); end
            end
            ...
            A  = @(I) ifft2(reshape(repmat(I.*Q1,[1 L]), size(I,1), size(I,2), L).*conj(Masks))...
                .*reshape(repmat(Q2,[1 L]), size(I,1), size(I,2), L) * size(I,1) * size(I,2);
            
            At = @(I) sum(fft2(reshape(repmat(conj(Q2),[1 L]), size(I,1), size(I,2), L).*I)...
                .*reshape(repmat(conj(Q1),[1 L]), size(I,1), size(I,2), L).*Masks , 3);
...
save('result_fresnel_success','results_mat'); 